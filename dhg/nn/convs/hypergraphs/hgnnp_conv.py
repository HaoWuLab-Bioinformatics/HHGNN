...
from dhg.attention.ECAAttention import ECAAttention
#from dhg import attention
from dhg.attention.ExternalAttention import ExternalAttention
...
class HGNNPConv(nn.Module):
    ...
    def __init__(
        ...
        self.att = ECAAttention(kernel_size=6)
        self.Conv1d1 = nn.Conv1d(1, 16, 1)
        self.Sigmoid1=nn.Sigmoid()
        self.drop1=nn.Dropout1d()
        self.Conv1d2=nn.Conv1d(16,32,2)
        self.Sigmoid2=nn.Sigmoid()
        self.drop2 = nn.Dropout1d()
        self.att = ECAAttention(kernel_size=6)
        ...
        self.hidden = torch.nn.Linear(in_channels, out_channels, bias=bias)
    ...
    def forward(self, X: torch.Tensor, hg: Hypergraph) -> torch.Tensor:
        ...
        if self.bn is not None:
            X = self.bn(X)
        X = hg.v2v(X, aggr="mean",drop_rate=0.05)
        if not self.is_last:
