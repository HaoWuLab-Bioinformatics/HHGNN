...
def a_layer(x,units):
    ...
    tf.add_to_collection('l2_reg', tensorflow.nn.l2_loss(W))
...
def bi_layer(x0,x1,sym,dim_pred):
    if sym == False:
        ...
        tf.add_to_collection('l2_reg', tensorflow.nn.l2_loss(W0p))
        tf.add_to_collection('l2_reg', tensorflow.nn.l2_loss(W1p))
